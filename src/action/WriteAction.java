...
public class WriteAction extends Action {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		result = result ? (candidateDTO = candidateDAO.doWrite(candidateDTO)) != null : false;
		moveObject = moveObject == null ? (result ? null : BackJSP(response)) : moveObject;
		
		result = result ? fileUpload(multipartRequest, candidateDTO, saveDirectory) : false;
		moveObject = moveObject == null ? (result ? null : BackJSP(response)) : moveObject;

		result = result ? fileDelete(multipartRequest, saveDirectory) : fileDelete(multipartRequest, saveDirectory) && candidateDAO.doDelete(candidateDTO);
